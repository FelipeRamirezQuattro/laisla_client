...
export function CascadeWarningModal({ isOpen, affectedPacks, affectedRecipes, onConfirm, onCancel, loading }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-island-dark bg-opacity-60 backdrop-blur-sm" onClick={onCancel} />
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md p-6">
        <div className="flex items-start gap-3 mb-4">
          <AlertTriangle size={24} className="text-warning shrink-0 mt-0.5" />
          <div>
            <h3 className="font-body text-lg font-semibold text-island-dark">Actualización en cascada</h3>
            <p className="text-sm text-island-dark/70 mt-1">
